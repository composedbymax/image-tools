...
newLayerTolerance.oninput = () => {
    newToleranceValue.textContent = newLayerTolerance.value;
    previewNewLayer();
};
newLayerHue.oninput = () => {
    newHueValue.textContent = newLayerHue.value;
    previewNewLayer();
};
newLayerUnify.oninput = () => {
    newUnifyValue.textContent = newLayerUnify.value;
    previewNewLayer();
};
newLayerColor.addEventListener('input', previewNewLayer);
...
function initWebGL() {
    ...
    const fs = `
        ...
        vec3 rgb2hsl(vec3 c){
            ...
            if(maxc != minc) {
                ...
                h = maxc == c.r ? (c.g - c.b) / d + (c.g < c.b ? 6.0 : 0.0) :
        ...
        void main(){
            ...
            vec3 color = tex.rgb, outColor = vec3(0.0);
            bool matched = false;
    ...
    const vsShader = compile(gl.VERTEX_SHADER, vs),
          fsShader = compile(gl.FRAGMENT_SHADER, fs);
...
function processImageWithLayers(layers) {
    ...
    const cols = [],
          ...
          hues = [],
          unify = [];
    layers.slice(0, MAX_LAYERS).forEach(l => {
    ...
    gl.uniform1i(uLoc.u_numLayers, Math.min(layers.length, MAX_LAYERS));
    gl.drawArrays(gl.TRIANGLES, 0, 6);
    downloadButton.disabled = false;
}
function processImage() {
    processImageWithLayers(colorLayers);
}
function previewNewLayer() {
    if (!originalImage || !gl) return;
    const tempLayers = [...colorLayers];
    ...
    processImageWithLayers(tempLayers);
}
